...
**Русский** · [English](README.en.md) · [История изменений](CHANGELOG.md#русский)
...
Профили AI-черновика общие для всех чатов. Выбор профиля применяет режим, endpoint, ключ, модель и температуру. Ручное изменение полей не перезаписывает профиль — для этого нужно нажать `Обновить выбранный`.

---

### Сохранение профилей изображений и AI-черновика

«Обновить выбранный» перезаписывает выбранный профиль текущими параметрами. «Сохранить как новый…» запрашивает название и сохраняет отдельный профиль. «Удалить профиль…» удаляет выбранную запись после подтверждения, сохраняя текущее подключение. Без выбранного профиля обновление и удаление недоступны.
...
После применения набора черновика названия выбранных стиля и макета обновляются в редакторе и сводке настроек. При удалении набора можно также удалить связанные сохранённые стиль и макет, только если они не используются в других наборах, текущих настройках и сохранённых черновиках. При прямом удалении стиля или макета подтверждение показывает все затронутые ссылки.
